...
local function resolve_cross(child, dir, content_cross, align, main_size)
    local cs = child.style or {}
    local raw = (dir == "row") and cs.height or cs.width
    ...
    local cross = (dir == "row") and ih or iw
...
function M.layout_fixed(node, x, y, w, h)
    ...
    local content_main = (dir == "row") and node._box.content_w or node._box.content_h
    local content_cross = (dir == "row") and node._box.content_h or node._box.content_w
